...
import { Loader2, Package, Search, ChevronDown, ChevronUp, MapPin } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function GlobalLocationsView({ spreadsheetId, dynamicTabs }: { spreadsheetId: string, dynamicTabs: string[] }) {
  const [loading, setLoading] = useState(true);
  const [products, setProducts] = useState<Product[]>([]);
  const [locations, setLocations] = useState<Record<string, Product[]>>({});
  ...
  useEffect(() => {
    ...
    for (const p of products) {
      ...
      let locStocks: Array<{location: string, qty: string}> = [];
      try {
        if (p.locations_stock) {
           locStocks = JSON.parse(p.locations_stock);
        }
      } catch(e) {}

      if (locStocks.length > 0) {
        for (const ls of locStocks) {
           const loc = ls.location || "Unassigned Location";
           if (!grouped[loc]) grouped[loc] = [];
           // duplicate the product object but override stock and location specifically for this grouped view
           grouped[loc].push({ ...p, stock: ls.qty, location: loc });
        }
      } else {
        const rawLoc = p.location || "Unassigned Location";
        // Check for backwards compatibility legacy string format "LocA: 10\nLocB: 20"
        if (rawLoc.includes(": ") && rawLoc.includes("\n")) {
           const lines = rawLoc.split("\n");
           for (const line of lines) {
              const [locName, qty] = line.split(": ");
              const loc = locName || "Unassigned Location";
              if (!grouped[loc]) grouped[loc] = [];
              grouped[loc].push({ ...p, stock: qty || p.stock, location: loc });
           }
        } else {
           if (!grouped[rawLoc]) grouped[rawLoc] = [];
           grouped[rawLoc].push(p);
        }
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[300px]">
        ...
        <p className="text-slate-500">Loading all inventory data by location...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-2xl font-semibold text-slate-800">Storage Locations</h2>
          <p className="text-sm text-slate-500 mt-1">Browse and find all inventory items grouped by their physical storage location.</p>
        </div>
      ...
      <div className="space-y-4">
        {Object.keys(locations).length === 0 ? (
           <div className="p-8 text-center bg-white rounded-lg border border-slate-200 border-dashed text-slate-500">
             No locations or items found matching your search.
           </div>
        ) : (
          Object.keys(locations).map(loc => {
            const items = locations[loc];
            const isExpanded = expandedLocs[loc];
            return (
              <Card key={loc} className="overflow-hidden border-slate-200 shadow-sm">
                <div 
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-slate-800 text-lg">{loc}</h3>
                      <p className="text-xs text-slate-500 font-medium">{items.length} {items.length === 1 ? 'product' : 'products'} stored here</p>
                    </div>
                  </div>
                        ...
                        <tbody className="divide-y divide-slate-100 bg-white">
                          {items.map((item, idx) => (
                            <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                              ...
                              <td className="px-6 py-3 text-slate-600 font-medium">{item.location || "Not set"}</td>
